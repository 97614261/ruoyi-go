...
## 2026-09-16 生成物运行时数据完整性以真实数据库验收为准

**当前边界**：生成模型对物理主键输出 GORM `primaryKey`，自增主键同时输出
`autoIncrement`，保证主子表新增主记录后能取得数据库 ID，再批量写入正确外键。生成实体继续
兼作请求对象，但 `binding:"required"` 只用于字符串；数值和布尔字段的零值不能被当作“未传”，
树根 `parent_id = 0` 必须可新增。存在 `update_by`、`update_time` 时，更新 map 无条件包含这两列，
使 service 设置的审计值真实落库。

树模板删除前先按主键批量读取待删树编码，再用一次查询判断是否存在未包含在本次删除集合中的
直接子节点；存在则返回“存在子节点，不允许删除”。父节点和全部子节点同批删除仍允许。父节点
不存在、重复编码、自身/后代成环和层级安全上限属于可安全展示的业务校验，统一返回
`errs.BizError`；数据库读取失败仍隐藏内部细节。

**为什么**：2026-09-16 使用当前 MySQL 真实生成并启动 `crud/tree/sub` 三类模块后，发现纯渲染
和编译测试没有覆盖五个运行时问题：树根的合法零值被 `required` 拒绝；缺失主键标签导致 GORM
不回填主表 ID，明细以外键 0 插入；单删树父节点留下孤儿；更新审计值只在内存赋值而未进入
`Updates`；防环虽然生效，但客户端只能看到通用系统错误。修复后，同一真实表上重新生成，Go
全量测试和构建、Vue3 生产构建均通过；运行时复测确认根节点新增、主子外键回填、更新审计、
树删除保护和具体校验消息全部通过。

**受控差异**：数值/布尔必填不再用结构体零值校验，避免把合法零值误判为缺失；若具体业务
必须区分字段是否出现，应为该模块生成或手写指针 DTO。树数据完整性错误会向客户端显示稳定的
业务文案，属于比通用“系统异常”更具体的可观察错误语义，但不暴露 SQL 或数据库错误。

**禁止回退**：不得删除生成主键的 GORM 标识，不得恢复对值类型机械添加 `required`，不得让
主子新增依赖未验证的零值主键，不得遗漏更新审计列。树删除不得逐 ID 循环查库、不得留下孤儿，
也不得把可预期的树校验重新包装为内部错误。模板测试必须分别定位更新函数和删除函数，不能只
在整个文件中搜索关键字；真实验收必须至少断言子项外键、审计字段和删除后的数据状态。

**相关位置**：`internal/service/gen_render.go`、`internal/service/gen_render_test.go`、
`docs/CONVENTIONS.md`、`docs/RUNTIME_AUDIT.md`。

---

## 2026-09-15 Go 原生代码生成器纳入冻结契约

**当前边界**：实现 Java/Vue3 已使用的 `/tool/gen/**` 13 条接口，并把它们纳入路由、方法、
参数位置、权限标识和响应层级冻结。生成器继续复用 `gen_table`、`gen_table_column` 和当前库的
`information_schema`，不新增表、列、约束或索引。生成物改为适配本仓库的 Go 四层代码、
同包自动路由登记、Vue 页面/API、菜单 SQL 和使用说明；不机械生成 Java 后端。

元数据导入按表和列各一次批量读取，配置写入使用事务；同步数据库按物理列名保留仍存在字段
的生成配置，只新增新列、删除已消失列。建表入口只接受当前库内的 `CREATE TABLE`，限制
1MB/20 张；ZIP 在内存完整生成后再写响应。自定义路径沿用 Java 的默认关闭语义，显式开启后
仍只能写入 `gen.outputRoot`，并拒绝目标文件符号链接。

`crud`、`tree`、`sub` 三类选择不允许出现“能保存但生成物语义残缺”：树形页面通过稳定分页
拉齐所有行后组树，主子表详情同时读取子表，新增、修改替换子项和批量删除均放在同一事务。
主子键类型、树字段归属、生成标识符和字典标识符在生成前再次校验。自定义写盘在 Windows
覆盖旧文件时先同目录备份，替换失败恢复旧文件，避免一次失败直接丢失已有代码。

生成的业务模块同时提供 Excel 导出：查询仍复用生成的过滤条件，但最多读取
`MaxExportRows + 1` 条用于拒绝超限结果，并经过全局导出并发中间件；模板同步生成后端路由、
Excel 字段标签、前端按钮和菜单权限。树模板的父节点候选使用无筛选条件的独立分页加载，
避免当前列表搜索条件把合法父节点隐藏；修改节点时从候选中移除自身整棵子树。前端过滤不是
数据完整性边界，生成的 service 会在单实例领域锁内调用 repository，沿父链迭代检查父节点
存在性，并拒绝自身、后代和已有环。repository 一次读取编码和父编码两列，然后在内存追溯，
不会形成沿层级循环查库的 N+1。该实现不依赖递归 CTE，兼容项目支持的 MySQL 5.7；
最多追溯 10000 层，超过上限按非法树处理。树编码和父编码必须使用相同的整数或字符串类型。

**为什么**：项目规范已经要求模块化 CRUD 由生成器产出，继续排除会让“替代 Java”永远留着
13 条真实功能缺口。直接复制 Java Velocity 模板会产出无法用于 Go 项目的代码；Go 原生模板
既保留现有管理页面和接口，又让生成物遵守 repository 独占数据库连接、统一响应、稳定分页、
权限挂路由和单实例写锁等本项目规则。SQL 白名单和输出根限制用于避免管理员页面退化成任意
SQL/任意文件写入入口。

**受控差异**：生成文件内容和目录是 Go 原生布局，不与 Java ZIP 逐字节一致；普通接口形状、
权限和下载协议保持一致。生成器不自动执行菜单 SQL，也不自动把生成物复制进参考 Vue 项目。
代码生成写操作尚需隔离 MySQL 环境做完整副作用对拍，不能拿 143/143 静态路由匹配冒充完成。

**禁止回退**：不得重新把代码生成器排除在路由审计外；不得允许任意 SQL、跨 `outputRoot`
写入、生成后自动执行菜单 SQL，或让 handler/service 直接持有 `*gorm.DB`。不得把主子表退化
成只保存主表、让树模板静默截断到第一页、只靠前端过滤防环，或生成不受行数/并发保护的导出。
修改模板时必须保留 Go 语法渲染测试、三类模板渲染测试、SQL 分句/拒绝测试和 13 条路由权限
对齐检查。

**相关位置**：`internal/model/gen.go`、`internal/repository/gen.go`、
`internal/service/gen*.go`、`internal/handler/gen.go`、`internal/router/router.go`、
`cmd/routeaudit/main.go`、`docs/API_ROUTE_COVERAGE_CURRENT.md`。

---

## 2026-09-08 权限版本跨重启失效、补偿限流与独立契约基线

**当前边界**：每个用户新增无 TTL 的 Redis 权限版本
`login_user_permission_version:<userId>`，并写入 `LoginUser.permissionVersion`。角色、菜单、
用户角色/部门等权限变化先固定受影响用户 ID，再推进版本，Redis 失败则拒绝数据库写入；
提交后主动刷新会话。登录的权限查询、版本读取和会话创建由同一权限锁串行化；鉴权发现旧
版本时从数据库懒刷新并 CAS 写回，任何依赖失败都按未登录拒绝。权限版本与会话代数分离：
前者允许无感刷新，后者用于停用、删除和改密后的强制退出。

进程内撤销补偿降级为会话 key 清理优化，不再是跨重启安全依据。队列最多保留 100000 个
用户，每批 200，错误详情最多 20 条，Redis 脚本按 Pipeline 执行，重试从 250ms 指数退避到
5s；日志暴露 pending、重试次数和最老等待时间。在线用户列表的 5000 上限改为按 SCAN key
计数，MGET 前裁剪最后一批，损坏 value 不能绕过。定时任务修改严格限制 `status/concurrent`
为 `0/1`、`misfirePolicy` 为 `0/1/2/3`，job 领域锁由可注入操作层的阻塞并发测试验证，不再
依赖源码字符串搜索。Go 仓库内提交 `contract_identifiers.json`，普通单测始终双向核对；兄弟
Java/Vue 仓库存在时再与同一基线比较，存在但路径或提取结果为空必须失败。

**为什么**：提交角色变更后才查询成员时，一次数据库故障会让旧权限继续使用；而只放在 Go
map 中的 pending 在崩溃或重启后消失。固定 250ms 全量复制/重试和逐用户错误会在十万成员
角色故障时放大 CPU、网络和日志。在线上限若在 JSON 成功解码后计数，大量损坏 key 仍会遍历
完整 Redis。源码字符串只能证明写了锁调用，不能证明数据库和 scheduler 位于同一临界区；
跨仓库测试直接 Skip 也不能保护只检出 Go 仓库的 CI。

**受控差异**：Java 不维护权限版本，权限变化后的旧会话处理更弱；Go 的版本推进失败会拒绝
本次权限数据库变更，鉴权可能在首次命中旧版本时多一次数据库读取。接口字段、成功响应、
数据库结构和数据均不改变；Redis 增加一个冻结前缀，Java/旧 Go 硬切清理由三个扩为四个。
任务非法枚举恢复 Java 的失败语义。

**禁止回退**：不得把权限正确性重新建立在进程内 pending 上，不得先提交权限数据库变更再
查询成员或推进版本，不得给权限版本设置会早于旧 JWT 消失的 TTL。不得按有效 JSON/过滤命中
计在线扫描上限，不得恢复逐用户 Redis 往返、无限错误详情、固定高频重试或源码字符串锁测试。
契约 manifest 只能由显式维护流程更新，普通测试不得自动覆写。

**相关位置**：`pkg/redisx/keys.go`、`internal/model/login.go`、
`internal/service/token.go`、`internal/service/permission_recovery.go`、
`internal/service/online.go`、`internal/service/role.go`、`internal/service/job.go`、
`internal/service/testdata/contract_identifiers.json`、`docs/DEPLOYMENT.md` 及其测试。

---

## 2026-09-03 定时任务服务端控制字段、权限失败补偿与批量传播

> ⚠️ **状态导航：进程内 pending 作为安全依据、固定重试和源码扫描锁测试已被 2026-09-08
> 「权限版本跨重启失效、补偿限流与独立契约基线」取代。** 本条的定时任务服务端控制字段、
> 批量权限装载和禁止无界 Pipeline 结论仍然有效。

**当前边界**：新增定时任务无条件清空客户端 `jobId` 并强制 `status=1`，启用仍只允许通过
`monitor:job:changeStatus` 接口；缺省 `misfirePolicy` 使用 Java `SysJob` 字段初始值 `0`。
新增、修改、状态切换、删除和立即执行统一持有 job 领域锁，锁覆盖数据库读取/写入与内存
scheduler 更新。

角色/菜单权限批量传播先把受影响用户加入进程内待补偿集合，再按每批 200 个用户读取会话
索引、每批 100 个 token 执行 MGET，并用固定查询数批量加载用户、部门、角色和权限。任何
索引读取、MGET、权限加载或会话 CAS 写回失败，都改用独立 2 秒 context 撤销该用户全部
会话；Redis 暂时不可用时由单个、容量为 1 的合并通知队列驱动重试。撤销成功前，认证入口
对待补偿用户按未登录拒绝。服务关闭时停止该 worker。

在线用户列表的 5000 上限改按过滤前的实际扫描数计算。契约测试自动扫描 Go 路由中的
`HasPermission(...)`；存在兄弟参考仓库时，还会双向核对 Java 基线 SQL 权限和 Vue
`useDict(...)` 字典类型。

**为什么**：Vue 新建任务实际发送 `status=0`，旧实现允许只有新增权限的账号顺带启用任务；
客户端还能提交复合主键中的 `jobId`。并发 Update/Delete 也可能让数据库已删除但 scheduler
仍保留幽灵任务。权限传播原先在 5 秒 context 超时后继续复用同一 context 撤销，撤销必然
立即失败；索引首批失败还会直接返回，旧权限会话可以继续续期。大角色一次性 Pipeline 且
逐在线用户查询数据库，成本与用户数线性增加。在线列表按过滤命中数限流则可被零命中过滤
绕过。手工抽查一个字典/权限也无法发现新增静态引用漏登记。

**受控差异**：Java 新建任务同样会强制暂停，本项恢复对齐；`misfirePolicy=0` 也恢复 Java
缺省语义。权限失败时临时拒绝受影响用户、在线列表达到扫描上限后截断是安全/资源加固，
请求和响应结构、数据库结构、数据及既有 Redis 前缀均未改变。

**禁止回退/外推**：不得信任新增请求中的 ID/状态，不得把 job 数据库写与 scheduler 更新
拆出同一领域锁。安全撤销不得复用失败 context，补偿成功前不得放行旧权限会话。不得恢复
无界索引 Pipeline、逐用户权限查询或按过滤命中数计算扫描上限。反向索引一定比 SCAN 快尚
无三组实测数据，不得把本次正确性/上界改造写成性能结论；仍需按 1000/100、100000/100、
100000/5000（成员/在线）压测。

**相关位置**：`internal/service/job.go`、`internal/service/write_locks.go`、
`internal/service/token.go`、`internal/service/permission_recovery.go`、`internal/service/online.go`、
`internal/repository/user.go`、`internal/repository/menu.go`、`cmd/server/main.go` 及其测试。

---

## 2026-09-03 关联写入、契约标识和提交后传播统一收口

> ⚠️ **状态导航：权限传播的提交后顺序已被 2026-09-08 决议收紧为“先固定用户并推进
> Redis 权限版本，再写数据库，提交后主动刷新”。** 本条的领域锁、缓存失效和契约标识保护
> 继续有效。

**当前边界**：保持单台服务器、单个 Go 进程，不修改数据库结构、索引或数据。进程内领域锁
从唯一性新增扩展到用户、角色、岗位、参数、字典、部门、菜单的全部关联写路径：关联存在性
校验、删除前引用检查和数据库写入处于同一临界区。多领域操作固定按“用户 → 角色 → 岗位 →
部门”和“角色 → 菜单”获取；字典类型和字典数据共用一个锁，避免类型改名/删除与数据写入
互相穿透。

角色权限传播不再扫描全站 Redis 会话，而是从 `sys_user_role` 查询未删除的当前成员，再按
`login_user_sessions:<userId>` 反向索引刷新。菜单、角色、用户资料及授权变更的数据库提交后
刷新，以及参数、字典写入后的缓存失效，统一使用保留 context value、脱离客户端取消并自带
5 秒 deadline 的 post-commit context；MGET 失败向上传递，不允许跳过整批后静默成功。

契约写保护覆盖七个代码配置键、Vue 消费的十个字典类型和 Java/Vue/Go 路由引用的菜单权限
标识。配置键、字典类型和既有权限标识不能无迁移改名或删除；契约配置必须保持系统内置。
验证码/注册开关、初始密码修改开关、密码有效天数、密码字符策略和初始密码在写入时校验，
读取历史非法值时也显式失败，不再把拼写错误或解析失败静默解释成关闭安全功能。

MySQL DSN 强制 `parseTime=true` 和 `types.Location`；`jwt.header` 只接受当前实际消费的
`Authorization`，`captcha.type` 只接受 `math/char`。操作日志的 JSON 响应旁路缓冲最多保留
2000 字节，并在多字节字符边界回退到合法 UTF-8。

**为什么**：原锁只保护重复业务标识，关联记录仍可能在“校验通过、写入发生”之间被另一请求
删除或改名；字典类型与数据分开写时也可能让缓存和引用穿透。按角色扫描全站会话让一次局部
变更的成本随全部在线人数增长。客户端在数据库提交后断开会取消原请求 context，可能导致
权限只刷新一部分或缓存未失效。契约标识此前只有文档约束，后台仍能把代码或 Vue 按名字读取
的行改坏。DSN、配置枚举和响应捕获问题则会把错误推迟到第一次业务查询、运行分支或大响应，
比启动期/写入期显式失败更难定位。

**受控差异**：Java 后台允许修改这些契约标识和写入部分非法配置值；Go 现在拒绝这类会让
既有消费者静默失效的操作。接口路径、参数位置、成功响应形状、数据库和 Redis 前缀不变。

**禁止回退**：不得恢复关联校验与写入分离、反向锁序、按角色扫描全站会话、提交后直接复用
请求取消信号、MGET 错误静默跳过，或只靠文档保护契约标识。新增代码/前端静态配置键、字典
类型或权限标识时必须同步契约集合和测试。不得取消 DSN 时间强制项、启动配置枚举校验、响应
捕获上限或 UTF-8 边界处理。若启动第二个进程，本条进程锁结论自动失效，必须先引入数据库
约束或可靠跨进程锁。

**相关位置**：`internal/service/write_locks.go`、`internal/service/user.go`、
`internal/service/role.go`、`internal/service/dict.go`、`internal/service/contract_data.go`、
`internal/service/postcommit.go`、`internal/service/token.go`、`internal/repository/role.go`、
`internal/repository/db.go`、`internal/config/config.go`、`internal/middleware/operlog.go` 及其测试。
...
> ⚠️ **状态导航：批次上限、批量权限装载和失败补偿已被 2026-09-03
> 「定时任务服务端控制字段、权限失败补偿与批量传播」扩展。** 不回退全站 SCAN、只信任
> 新版索引会话和部署硬切前提仍然有效。
...
> ⚠️ **已被 2026-09-15「Go 原生代码生成器纳入冻结契约」取代。** 当时“模板尚未稳定、
> 暂不实现”的背景保留为历史；当前模块和规则已稳定，生成器已经实现。
