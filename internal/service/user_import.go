...
func ImportUsers(ctx context.Context, operator *model.SysUser, r io.Reader, updateSupport bool, operatorName string) (string, error) {
	users, rowErrors, err := excelx.Import[model.SysUser](r, "")
	if err != nil {
	...
	if len(users) > maxImportRows {
		return "", errs.Newf("单次最多导入 %d 条数据，当前 %d 条", maxImportRows, len(users))
	}
	...
	for _, rowErr := range rowErrors {
		failureNum++
		fmt.Fprintf(&failureMsg, "<br/>%d、%s", failureNum, rowErr.Error())
	}

	for i := range users {
		user := users[i]
		if err := importOneUser(ctx, operator, &user, updateSupport, operatorName, hashed); err != nil {
			failureNum++
			fmt.Fprintf(&failureMsg, "<br/>%d、账号 %s 导入失败：%s", failureNum, user.UserName, messageOf(err))
			continue
		}
		successNum++
		fmt.Fprintf(&successMsg, "<br/>%d、账号 %s 导入成功", successNum, user.UserName)
	}
...
func importOneUser(ctx context.Context, operator *model.SysUser, user *model.SysUser,
	updateSupport bool, operatorName, hashedPassword string) error {

	if strings.TrimSpace(user.UserName) == "" {
		return errs.New("登录名称不能为空")
	}

	existing, err := repository.SelectUserByUserName(ctx, user.UserName)
	if err != nil {
		return err
	}
	...
	if existing == nil {
		...
		if err := validate.Struct(user); err != nil {
			return errs.New(firstValidationMessage(err))
		}
		if user.DeptID != nil {
	...
	if err := CheckUserDataScope(ctx, operator, existing.UserID); err != nil {
		return err
	}
	if user.DeptID != nil {
		if err := CheckDeptDataScope(ctx, operator, *user.DeptID); err != nil {
